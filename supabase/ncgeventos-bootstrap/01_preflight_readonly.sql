...
--  Verifica estado de origen (ncgconstructora) y destino (ncgeventos) antes de tocar nada.
...
-- Counts de objetos en `ncgconstructora` — baseline para comparar luego del clonado.
...
SELECT 'policies' AS tipo,
       COUNT(*)::int AS total
  FROM pg_policies
 WHERE schemaname = 'ncgconstructora';

SELECT 'functions' AS tipo,
       COUNT(*)::int AS total
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
 WHERE n.nspname = 'ncgconstructora';

SELECT 'triggers' AS tipo,
       COUNT(*)::int AS total
  FROM information_schema.triggers
 WHERE trigger_schema = 'ncgconstructora';
...
-- Funciones con referencias literales al string 'ncgconstructora' (útil para
...
-- Triggers con referencias literales a 'ncgconstructora'.
...
-- Policies con referencias literales a 'ncgconstructora'.
...
-- Confirmar presencia de las tablas core en ncgconstructora.
SELECT table_name
  FROM information_schema.tables
 WHERE table_schema = 'ncgconstructora'
