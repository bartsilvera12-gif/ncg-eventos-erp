-- ============================================================================
--  07 — SEED ADMIN NCGEVENTOS (MODIFICADOR — riesgo medio)
--
--  RECORDATORIOS CRÍTICOS:
--    * auth.users es GLOBAL al proyecto Supabase — NO se duplica por schema.
--    * NO insertar en ncgconstructora.usuarios.
--    * NO insertar en otros schemas (public, zentra_erp, elevate, etc).
--    * La fila ERP del admin va ÚNICAMENTE en ncgeventos.usuarios.
--
--  PRE-SQL (NO ES SQL):
...
-- 1) Introspección de columnas de ncgeventos.usuarios (READ-ONLY).
SELECT column_name,
       data_type,
       is_nullable,
       column_default
...
-- 5) Verificar que NO existe ya en ncgeventos.usuarios (READ-ONLY).
...
--    1. Confirmar columnas reales de ncgeventos.usuarios.
--    2. Haber creado <AUTH_USER_ID> en auth.users vía Dashboard / Admin API.
--    3. Confirmar <EMPRESA_ID> existente en ncgeventos.empresas.
--
-- INSERT INTO ncgeventos.usuarios (
--   id,
--   auth_user_id,
--   empresa_id,
--   email,
...
-- SÓLO en ncgeventos.usuarios. Recorre dinámicamente todas las tablas
...
BEGIN
  ...
  LOOP
    ...
    IF v_total > 0 AND r.table_schema <> 'ncgeventos' THEN
      v_offending := v_offending || r.table_schema || '.' || r.table_name || ' ';
    END IF;
  END LOOP;
  IF length(v_offending) > 0 THEN
    RAISE NOTICE 'AUTH_USER_ID presente fuera de ncgeventos: %', v_offending;
  ELSE
    RAISE NOTICE 'OK: AUTH_USER_ID solo en ncgeventos.usuarios (o aun no insertado).';
  END IF;
END
$$ LANGUAGE plpgsql;
