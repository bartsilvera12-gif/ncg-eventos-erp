...
import {
  ensureSemillasSucursales,
  generarSlugDesdeNombre,
  type SucursalRow,
} from "@/lib/rrhh/sucursales-catalogo";
...
export async function GET(request: NextRequest) {
  try {
    ...
    await ensureSemillasSucursales(ctx.supabase, ctx.auth.empresa_id);
    ...
    return NextResponse.json(successResponse({ tipos: (data ?? []) as SucursalRow[] }));
  } catch (err) {
  ...
  try {
    ...
    if (error) {
      if (String(error.message).toLowerCase().includes("unique")) {
        return NextResponse.json(errorResponse("Ya existe una sucursal con ese identificador"), { status: 400 });
      }
