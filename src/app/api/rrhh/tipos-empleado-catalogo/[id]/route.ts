...
/** PATCH { nombre?, activo?, orden? } — no se puede cambiar slug. */
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if ((row as { es_sistema: boolean }).es_sistema) {
      return NextResponse.json(errorResponse("Los tipos de sistema no se pueden eliminar (desactívalos en su lugar)."), { status: 400 });
    }
