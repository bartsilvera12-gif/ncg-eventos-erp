...
import {
  ensureSemillasDepartamentos,
  generarSlugDesdeNombre,
  type DepartamentoRow,
} from "@/lib/rrhh/departamentos-catalogo";

/**
 * GET /api/rrhh/departamentos-catalogo
 *   - default: trae sólo activos (forms)
...
export async function GET(request: NextRequest) {
  try {
    ...
    await ensureSemillasDepartamentos(ctx.supabase, ctx.auth.empresa_id);
    ...
    return NextResponse.json(successResponse({ tipos: (data ?? []) as DepartamentoRow[] }));
  } catch (err) {
...
/**
 * POST /api/rrhh/departamentos-catalogo
  ...
  try {
    ...
    if (error) {
      if (String(error.message).toLowerCase().includes("unique")) {
        return NextResponse.json(errorResponse("Ya existe un departamento con ese identificador"), { status: 400 });
      }
