...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertBase = {
      ...
      pais:                 pais?.trim() || null,
      codigo_postal:        typeof codigo_postal === "string" ? (codigo_postal.trim() || null) : (codigo_postal ?? null),
      provincia:            typeof provincia === "string" ? (provincia.trim() || null) : (provincia ?? null),
      contacto_persona:     typeof contacto_persona === "string" ? (contacto_persona.trim() || null) : (contacto_persona ?? null),
      fecha_alta:           typeof fecha_alta === "string" && fecha_alta.trim() ? fecha_alta.trim() : (fecha_alta ?? null),
      fecha_baja:           typeof fecha_baja === "string" && fecha_baja.trim() ? fecha_baja.trim() : (fecha_baja ?? null),
      regimen_fiscal:       typeof regimen_fiscal === "string" ? (regimen_fiscal.trim() || null) : (regimen_fiscal ?? null),
      forma_pago:           typeof forma_pago === "string" ? (forma_pago.trim() || null) : (forma_pago ?? null),
      iban:                 typeof iban === "string" ? (iban.trim() || null) : (iban ?? null),
      bic_swift:            typeof bic_swift === "string" ? (bic_swift.trim() || null) : (bic_swift ?? null),
      condicion_pago:       condicion_pago?.trim() || null,
      moneda_preferida:     moneda_preferida === "USD" ? "USD" : moneda_preferida === "EUR" ? "EUR" : "GS",
