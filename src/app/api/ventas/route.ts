...
function mapItems(rows: VentaItemRow[]): LineaVenta[] {
  return rows.map((r) => ({
    ...
    tipo_precio: (r.tipo_precio === "mayorista" || r.tipo_precio === "costo"
      ? r.tipo_precio
      : "minorista") as TipoPrecioVenta,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const ventas: Venta[] = ventasRows.map((r) => {
      ...
      const proyTitulo = Array.isArray(r.proyectos) ? r.proyectos[0]?.titulo : r.proyectos?.titulo;
      return {
        id: r.id,
