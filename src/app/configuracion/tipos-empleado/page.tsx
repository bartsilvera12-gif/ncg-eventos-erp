...
/**
 * Configuración · Tipos de empleado
 *
 * CRUD del catálogo de roles operativos que aparecen en /rrhh/tipos-empleado
 * cuando se asigna un tipo a un empleado. Mismo patrón que /configuracion/crm
 * (tipos de servicio de cliente): seed de sistema editable pero no
 * eliminable, agregables/desactivables sin tocar DB manualmente.
 */
...
export default function TiposEmpleadoConfigPage() {
  ...
  async function cargar() {
    ...
    setError(null);
    try {
      const r = await fetchWithSupabaseSession("/api/rrhh/tipos-empleado-catalogo?all=1", { cache: "no-store" });
      const j = await r.json();
      if (j.success) setTipos(j.data.tipos ?? []);
  ...
  async function actualizar(id: string, patch: Partial<{ nombre: string; activo: boolean; orden: number }>) {
    const r = await fetchWithSupabaseSession(`/api/rrhh/tipos-empleado-catalogo/${id}`, {
      method: "PATCH",
  ...
  async function borrar(t: TipoRow) {
    if (t.es_sistema) return;
    if (!confirm(`¿Eliminar el tipo "${t.nombre}"? Esta acción no se puede deshacer.`)) return;
    const r = await fetchWithSupabaseSession(`/api/rrhh/tipos-empleado-catalogo/${t.id}`, { method: "DELETE" });
    const j = await r.json();
  ...
  async function crear() {
    ...
    if (!nombre) return;
    const r = await fetchWithSupabaseSession("/api/rrhh/tipos-empleado-catalogo", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Editá la lista de roles operativos (Obrero, Capataz, Soldador…) que aparece en la pantalla de asignación de tipo de empleado."
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-5 py-3">
          <input
            ...
            placeholder="Nuevo tipo (ej: Encofrador)"
          ...
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-400">Sin tipos.</td></tr>
              ) : (
                tipos.map((t) => (
                  <tr key={t.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-5 py-3 text-right">
                      <button
                        ...
                        title={t.es_sistema ? "Los tipos del sistema no se eliminan, solo se desactivan" : "Eliminar"}
